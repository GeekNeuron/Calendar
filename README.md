...
# --- کلاس Worker برای پردازش کامل کتاب در پس‌زمینه ---
...
# --- کلاس نوار پیشرفت قابل کلیک با رنگ واکنش‌گرا ---
...
class EpubReader(QMainWindow):
    ...
    def on_book_data_loaded(self, result):
        ...
        if 'error' in result:
            self.text_display.setHtml(f"<h1>خطا در باز کردن کتاب</h1><p>{result['error']}</p>")
            return
